  ...
  try {
    if (action === 'start') {
      ...
      if (pose === 'sitting_side') {
        ...
        postureDescription = "standing up on its own on a soft fluffy rug in a natural, candid, relaxed way, as if caught in a spontaneous happy moment (like just starting to toddle or shift weight). The body is turned very slightly to one side (a gentle 3/4 angle, NOT a stiff dead-straight frontal 'attention' pose), the weight resting a little more on one leg with one foot placed slightly forward or turned. Both arms are loose and relaxed in clearly different, asymmetric positions (for example one arm swinging slightly out or up and the other lower by the side), not identical and not hanging stiffly. The head is slightly tilted, looking happily towards the camera. Balancing freely by itself, NOT holding onto anything. There is NO sofa, NO cushion, NO pillow, NO rolled-up blanket, NO stool, NO supporting hand and NO prop or object of any kind in the scene. Shot at the baby's eye level, horizontal camera angle, strictly NOT looking down from above. Absolutely NOT a stiff, rigid, symmetric, or standing-at-attention pose";
